  ...
  return (
    <div>
      <div>
       ...
       <div>
     <img src="https://banner.caratlane.com/live-images/40d09efd1fc94e73b56dcb5cd3ae7077.webp" alt="" />
    </div>
    <div style={{display:"flex"}}>
      ...
      <div>
       <div style={{padding:"60px 40px"}}><img src="https://assets.cltstatic.com/images/responsive/DefaultBanner_Desktop.webp" alt="" width={1100}/> </div>
       <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"30px",padding:"0px 40px"}}>
      {
        data.map((item)=>(
          <div key={item.id} className="card">
          <Link to={`/Gifts/${item.id}`}> <img src={item.avatar} alt="" width={500}/></Link>
          <h1>₹ {item.price}</h1>
          <h3>{item.name}</h3>
